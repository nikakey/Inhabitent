 ...
 * @copyright Business Hours Widget
 *
 * @wordpress-plugin
 * Plugin Name:       Business Hours Widget
 * Plugin URI:        @TODO
 * Description:       Business Hours Widget for the Inhabitent Theme
 * Version:           1.0.0
 * Author:            Veronika
 * Author URI:        https://github.com/nikakey
 * License:           GPL-2.0+
...
class Business_Hours_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array(
				...
				'description' => 'Business Hours Widget adds Business Hours info to the sidebar.'
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		$title = empty( $instance['title'] ) ? '' : apply_filters
		( 'widget_title', $instance['title'] );
		
		$monday_friday = empty( $instance['monday_friday'] ) ? '' : apply_filters
		( 'monday_friday', $instance['monday_friday'] );

		$saturday = empty( $instance['saturday'] ) ? '' : apply_filters
		( 'saturday', $instance['saturday'] );

		$sunday = empty( $instance['sunday'] ) ? '' : apply_filters
